...
class CompleteHierarchicalClustering(InitialClustering):
    ...
    def start_initial_clustering(self):
        ...
        distances, number_of_clusters, clusters = precalculate_distances(self.sea_level_anomaly_data,
        ...
        for clustering in all_clusters.values():
            ...
            save_clustering(clustering, len(clustering), self.out_dir)
...
def precalculate_distances(sea_level_anomaly_data: xarray.Dataset, distance_function: callable) -> (np.ndarray, int,
                                                                                                    {int: Cluster}):
    """
    ...
    :param sea_level_anomaly_data: xarray dataset with the sea level anomaly data.
    :param distance_function: Function to calculate the distance between two points. The function takes six arguments: the latitude and longitude of the first point, the timeseries of the first point, the latitude and longitude of the second point, and the timeseries of the second point.
    :return: A tuple of the distances matrix, the number of clusters, and a dictionary of clusters with their ids as keys.
    """
    # in the beginning each cluster is a single grid point
    clusters = {}
    grid_points = []
    cluster_id = 0
    for i in tqdm(range(sea_level_anomaly_data.latitude.size), desc="creating clusters"):
    ...
    distances = distances_between_all_pairs(cluster_pairs, distances, distance_function)
...
def distances_between_all_pairs(cluster_pairs: list[tuple[Cluster, Cluster]], distances: np.ndarray,
                                distance_function: callable) -> np.ndarray:
    """
    ...
    results = Parallel(n_jobs=-2, backend='multiprocessing', verbose=1)(
        delayed(calculate_distance)(*args, distance_function) for args in cluster_pairs)
